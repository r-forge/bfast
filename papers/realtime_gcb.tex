...
\hypersetup{%
  pdftitle = {Real-time disturbance detection in terrestrial ecosystems using satellite image time series -- Drought detection in Somalia as a proof-of-concept},
...
First, simulation results illustrate that disturbances are successfully detected in real-time, i.e., at the end of a time series, while being robust for seasonality and noise. However, data pre-processing in order to improve the quality and reliability of time series data remains essential to improve disturbance detection. Second, major drought related disturbance corresponding with most drought stressed regions in Somalia are detected from mid 2010 onwards and confirm proof-of-concept of the method. The method has the potential to be integrated within current operational early warning systems and is publicly available within the \emph{BFAST} package for {R} \citep{R}. It has the potential to detect a wide variety of disturbances (e.g. deforestation, flood damage, etc.) and analyse in-situ or satellite data time series of biophysical indicators from local to global scale since is fast, does not depend on thresholds or definitions and does not require time series gap filling. 
