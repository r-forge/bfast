...
We demonstrate the proof-of-concept of a multi-purpose near real-time disturbance detection method by analysing MODIS 16-day NDVI image time series. While the data and methods used are appropriate for proof-of-concept development for global scale disturbance monitoring, specific applications (e.g., drought or deforestation monitoring) will mandate integration within an operational early warning framework (e.g. \url{www.fews.net} or \url{earlywarning.usgs.gov}).  

Here, the disturbance detection is limited to areas (1)~with a minimum amount of vegetation cover by restricting the analysis to time series with a median NDVI $>0.2$ during the 2000--2011 period \citep{Vrieling:2011da} and (2)~with a stable history longer than two years (see Section~\ref{sec:DiscSim});
...
The method is (1)~tested in a controlled environment by simulating time series containing disturbances and (2)~demonstrated by applying it to MODIS NDVI
