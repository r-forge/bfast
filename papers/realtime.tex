...
\citet{Verbesselt:2010wo} focus on the question if and where there
are structural breaks in season/trend of a given observed sample
$t = 1, \dots, n$, we want to investigate a different questions:
Does the season/trend for future observations $t = n, n + 1, \dots$ still
conform with the season/trend established for the historical sample
$t = 1, \dots, n$? Thus, we want to monitor potential abnormality
(compared to previous observations) at the end of a time series in
real time.
...
\end{align}
%
where the intercept $\alpha_1$, slope $\alpha_2$ (both pertaining to trend), amplitudes $\gamma_1, \dots, \gamma_k$,
and phases $\delta_1, \dots, \delta_k$ (all pertaining to season) are the unknown parameters,
...
if all trend/season parameters in $\beta$ can be assumed to be stable for a given
...
Given that a stable season-trend model was estimated in an observed
time period, does it remain stable for future incoming
observations? If not it will indicated an abnormal structural change
in the process.
...
the size of the history sample, e.g., $h = n/4$ or $h = n$ etc.
...
The technical details for the boundary are somewhat more involved and based
on a so called functional central limit theorem. Here, we use the boundary
function $c \sqrt{2 \log_+ t/n}$, where $\log_+ x$ is $1$ for $x \le e$ and $\log x$
otherwise and $c$ is the critical value determines the significance level. The
critical values also depends on the choice of $h$ and the monitoring horizon $N$,
e.g., for $h = 0.25 n$ and $N = 10 n$, $c = 1.3409$ at the 5\% level.
See \cite{Leisch2000} and especially their Table~1 for more details.


\subsection{Selecting the stable history period}

A crucial assumption for the monitoring approach proposed above is that
the history period $t = 1, \dots, n$ itself is free of structural changes,
i.e., that the parameters $\beta$ are stable during this time. Only in that
case $\hat \beta$ is a consistent estimate (i.e., the season-trend structure
...
large abrupt shifts in the season-trend model in the history period. However,
if there are smaller gradual changes, \cite{Pesaran2002} show that this
breakpoint approach wastes power (due to the trade-off between bias and variance).
Hence, they suggest an alternative technique: Moving backward in time for
$t = n, n-1, n-2, \dots$ they consider cumulative 1-step ahead prediction errors
the season-trend model until this breaks down. The method is also known
as reversed ordered cumulative sum (CUSUM) of residuals, or ROC for short.
...
previous observation $s$ conforms with this. The suitably scaled residuals
...
as $\ell$, corrsponding to the last point up to which there is no sufficiently
...
\section{Conclusion}
Currently, real-time satellite images and their derivatives are routinely produced and published on the World wide Web (e.g. http://rapidfire.sci.gsfc.nasa.gov/) but methods for real-time monitoring and change detection are lacking. 
