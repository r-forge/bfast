...
Real-time monitoring forest and land cover disturbances are critical for addressing impacts on carbon storage, biodiversity, and other socio-ecological processes. Satellite remote sensing enables cost-effective and accurate monitoring at frequent time steps over large areas. Yet, methods to detect changes in real-time within newly captured satellite images are lacking. 

We are proposing a method to detect disturbances in real-time using satellite image time series by selecting a stable history period to model normal expect behaviour.
As such, enable differentiation between normal and abnormal change in real-time.
...
\centering
  ...
  \caption{Results from the simulation experiment (1000 iterations) illustrating the accuracy of time estimation of the detected breaks (RMSE expressed in time steps between images here 1 equals a 16-day period) while varying the amount of noise, magnitude of simulated break ($m$), and amount of data available in the monitoring period ($\#$). For more details please see description of the simulation experiment. }
  \label{fig:SimRMSE}
\end{figure}
...
\centering
  ...
  \caption{Spatial variation of the noise ratio is the study area. }
  \label{fig:SpatNoise}
\end{figure}
