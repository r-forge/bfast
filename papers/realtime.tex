...
We propose a generic time series based disturbance detection approach by modelling \emph{normal} expected historical behaviour and as such enable detection of \emph{abnormal} changes within newly acquired data.
...
variation and noise, and by adding changes with different magnitudes.  We demonstrate the proof-of-concept using the MODIS (Moderate Resolution Imaging Spectrometer) NDVI 16-day image composites from February 2000 until July 2011 for drought disturbance detection in Somalia. The severe drought occurring in Somalia (2010-2011) is selected as a proof-of-concept of the method. The NDVI is used, as a measure of vegetation dynamics and disturbances \citep{Pettorelli:2005ed, Potter2003, Myneni:1997ur}. The method can be applied on optimised drought indicators \citep{Funk:2009vf, Ghulam:2007td, Verbesselt2006} for operational implementation as a early warning tool. However, the method is developped for multiple purposes (e.g. forest disturbance detection) to automatically detect disturbances by analysing time series of biophysical in-situ and satellite derived indicators. 
