...
While the data and methods used here are appropriate for proof-of-concept development, other applications (e.g., drought monitoring, deforestation or oil spill detection) will mandate further testing. The main purpose of applying the real-time monitoring approach to MODIS 16-day NDVI time series is to illustrate a proof-of-concept of real-time disturbance detection. The method is is not developed specifically as a e.g. drought anomaly detection systems but more a global generic disturbance detection system. Specific climate- and satellite data based indices have been developed specifically for drought monitoring (reference). Here, however, we demonstrate how the method could be used as a generic early warning system by analysing MODIS 16-day NDVI time series:

First, Fig. \ref{fig:MedianNDVI} illustrates the spatial variation of the median of a NDVI time series (2000-2011) as a proxy for vegetation fractional cover during the period of analysis. The disturbance detection was limited to areas with a minimum amount of vegetation cover by restricting the analysis to time series with a median NDVI $>0.2$.  % This also corresponds to major agricultural areas in Somalia (reference FOA land cover map of MODIS land cover map) 
NDVI time series with no or very low NDVI values do not show any seasonality related to normal vegetation dynamics and are mainly influenced by variations soil properties. Other more specific vegetation indices are available for desert like conditions \citep[e.g. SAVI][]{Huete2002} however here we analysed NDVI time series as a proxy for global dynamics of photosynthetic capacity \citep{Myneni1995}. 

Second, Fig. \ref{fig:SpatLStableHist} illustrates the spatial variation of the length of the automatically identified stable history period (i.e. going backward in time from 2010). This result was used to restrict the analysis to regions with a stable history longer than 2 years. The length of the stable history is identified for modelling normal, stable data variation within the history period to enable differentiation from abnormal changes in the monitoring period. The method for identifying the stable history is based on the structural change detection methods \citep{Bai2003,Zeileis:2010tt} and is implemented to assess stability of the history period by verifying whether or not a structural change is occurring while stepwise going back in history. It is not implemented to determine accurately the time of change but for testing whether of not a change is occurring in the history period \citep{Zeileis2002}. The BFAST approach is developed to estimate time, magnitude, and direction of change within time series \citep{Verbesselt2009a}. The length of the stable history as shown in Fig. \ref{fig:SpatLStableHist} can be used to have a rough indication of where in history the last major disturbance occurred. More research, however, is require to validate and optimise the capacity of stable history identification for this purpose.

Third, Fig.~\ref{fig:SpatNoise} shows the spatial variation of the noise level ($\sigma$) in the study area. The lower noise levels correspond to the low vegetated areas or forested areas whereas the higher noise levels correspond to the semi-arid ecosystems (e.g. woodland and savannah). Higher noise levels in the semi-arid ecosystems can be explained by larger the inter-annual phenological shifts (e.g., shifts in the start and end of the growing season) due rainfall and temperature dynamics\citep{Verbesselt:2010wo}. The noise range (i.e. 0--0.15) in Fig.~\ref{fig:SpatNoise} corresponds to the noise range in Fig.~\ref{fig:SimNr} and confirms that the simulation experiment covered the range ecosystem dynamics present in the study area.

Finally, Fig.~\ref{fig:SpatTimeofChange} illustrates the time of the detected changes in 2010-2011 for areas where the stable history period is longer than 2 years and where the median of NDVI time series is larger than 0.2 NDVI. Results shown in Fig.~\ref{fig:SpatLStableHist},~\ref{fig:SpatNoise}, and~\ref{fig:SpatTimeofChange} are illustrated for three locations within the study area in Fig.~\ref{fig:realmon}. Fig.~\ref{fig:realmon} are examples for specific locations in the study area illustrating three cases; (1) a detected disturbance with the full history period, (2) a case were no disturbance is detected, (3) a detected disturbance for a time series with a shorter stable history period.
...
% nog iets over drought detection
% discussion over hoe de methode kan gebruikt worden en wat de tijd series nu betekenen?
...
Application on 16-day MODIS NDVI time series for drought detection in Eastern Africa, Somalia illustrate a proof-of-concept of the proposed time series based
