...
Most existing change detection methods smooth or interpolate data using one of many existing techniques (e.g. filtering, Fourier analysis)  \citep{Jonsson2002, Roerink2000} when dealing with noisy times series of remotely sensed data. For a given date, these methods typically require looking both backwards and forwards in time, negating use in real-time or forecast applications \citep{White2006}.  Also, time series smoothing and interpolation techniques model data and fill gaps based on assumptions of normal data variation which inhibits the detection of abnormal changes (i.e. disturbances, deforestation). Methods able to analyse non-gap filled time series are critical for real-time change detection.

Therefore, we propose a generic approach for real-time change detection of abrupt changes using time series data that deals with missing data and is independent of thresholds or change types. The following major research questions are answered in this paper:
...
Fig.~\ref{fig:SpatTimeofChange} illustrates the time of the detected changes in 2006. Many changes are detected in 2006 due to a drought that occurred around that time in combination with ongoing harvest operations. Fig.~\ref{fig:SpatLStableHist} shows the length of the stable history period and illustrates where and when changes occurred in the history period (e.g. a recent harvest operation). Results shown in Fig.~\ref{fig:SpatTimeofChange} and Fig.~\ref{fig:SpatLStableHist} are illustrated for three locations within the study area in Fig.~\ref{fig:realmonitoring} and Fig.~\ref{fig:shorthistory}.

Fig.~\ref{fig:realmonitoring} and Fig.~\ref{fig:shorthistory} are examples for specific locations in the study area illustrating the detected (1) disturbance and (2) the identified length of the stable history period. First, drought stress in 2006 is the main driver of the disturbance detected in the top graph of Fig.~\ref{fig:realmonitoring}, whereas a harvest operations explains the change detected in the bottom graph of Fig.~\ref{fig:realmonitoring}. Second, a different length of the automatically identified stable history is also visible in Fig.~\ref{fig:realmonitoring} (top versus bottom). A larger impact of a drought period in 2003 on the NDVI time series of a Pinus radiata plantation (top versus bottom graph) could potentially explain the shorter stable history period. Fig.~\ref{fig:shorthistory} illustrates an ongoing harvests operation during the history period which explain the shorter identified stable history period. Nonetheless a change is still detected in 2006 due the ongoing effects of the harvest operation.

\section{Discussion and further work}

Results of the simulation experiment and application on 16-day MODIS NDVI time series illustrate a proof-of-concept development and properties of the proposed time series based real-time monitoring approach. The change detection approach is a generic, data driven approach based on statistical principles which applicable to different types of time series. 

The method can be applied on to time series with a higher temporal resolution (e.g. hourly, daily or 8-day time series) to enable rapid response to detected disturbances. Noise level, available data in the monitoring period, and the magnitude of the disturbance that need to be detected are the drivers of the break detection and can be applied to different types of time series. Application of the  proposed method on the MODIS rapid response data (e.g. \url{http://lance.nasa.gov/}) or data available for geostationary satellites (e.g. \url{http://www.cmsaf.eu/}) could enable fast and automated detection of disturbances). 
...
Similar principles as shown in the simulation experiment are applicable to 


Firstly, for real time application


can also be applied on different types of time series. for example time series with a higher temporal resolution and similar amount of data (n) in combination of a certain level of noise, and magnitude of a break. The method is a data-driven method based on statistical principles which can be applied on different types of time series (e.g. daily, or 8-daily time series). As such the real-time change detection is improved and a rapid response for disturbance can be enabled. 

These results are illustrating when breaks are being detected within time series under different circumstances (i.e. $a,\sigma,n,m$) for a simulated 16-day time series. Further testing needs to be done for 8-day time series but
...
1. monitoring ~ noise, amplitude effect. Seasonal amplitude versus phenological changes
phenological change detection is also possible
Continuous modelling of the seasonality using the proposed concept also allows for relating remote sensing to a continual process of land surface phenology change instead of a single date extraction \citep{Verbesselt:2010wo,White2006}
% refer to the data simulation method as an ideal tool for validation of change detection methods in different environments. Further works is needed e.g. to test the real time monitoring approach when large seasonal shift (e.g. phenological variation in start-of-season) occur in the time series.

Further, many methods assume that a given mathematical function, such as piecewise logistic functions (Zhang et al., 2003), approximates true phenological development. In global, or even regional application this may be an untenable assumption, especially in cases of disturbance, defoliation, or NDVI curves with sharp peaks or broad plateaus (Potter et al., 2003).

Further testing is required to operationally implement the method for detection of other changes in other ecosystems.
...
data set selection - daily? - cloud effect discussion - sentinel or the dmc constellation - this analysis was done using 16-day data however in operational situation daily data could be used - further testing needs to be done to verify how this method can be optimized to handle daily data which potentially is more noise.
% modis rapid response, meteosat geostationary satellites, etc.

2. stable history period is a rough method for identification of a stable period - this needs to be used with care (e.g. in very noise time series -e.g. daily data) and further explored.
Alternatively a fixed history period can be manually chosen or an average for a region can be used a representing normal data variation (e.g. see Lhermitte)

%monitoring should be exported in an area where you now that the history period is more or less stable. e.g. if deforestation has happend in the year before the monitoring period than it should not be exported but been masked out from the analysis for example.

3. magnitude of change (how?) 
also keep in mind that positive and negative change are being detected so we should in a further fase verify in which coefficients (beta) of the most of the changes occur.

4. The real-time monitoring could be further improved by including other explanatory variables in the monitoring model (e.g. temperature, rainfall) which will contribute to model the data and determine what is normal versus what is abnormal. Furthermore, the monitoring model proposed here can also further improved to be used as a forecasting model. Similar to current real-time monitoring approach, forecasting requires a profound understanding of vegetation dynamics and vegetation interaction with the atmosphere, combined with proper data analysis techniques  \citep{LeiJi:2004kx}. % adding co-variates e.g. like temperature, precipitation might help differentiating a real change from seasonality.

5. Further testing towards operational implementation as an alarm system in forest ecosystems
...
%keep in mind that if there is a regrowth effect occurring and the ndvi is saturising we are detecting a change while in reality there is no real change.
...
1. no tresholds
2. no gap filling
3. full time series processing - a method that fully explores the potential of time series data.
