...
The method can be applied on to time series with a higher temporal resolution (e.g. hourly, daily or 8-day time series) to enable rapid response to detected disturbances. Noise level, available data in the monitoring period, and the magnitude of the disturbances are the drivers of the break detection valid for different types of time series. Application of the  proposed method on the MODIS rapid response data (e.g. \url{http://lance.nasa.gov/}) or data available for geostationary satellites (e.g. \url{http://www.cmsaf.eu/}) could enable fast and automated detection of disturbances). While the data and methods used here are appropriate for proof-of-concept development \citep{White2006}, other applications (e.g. deforestation monitoring in tropical forests or oil spill detection) will mandate further calibration and validation, as discussed below;

\begin{enumerate}[(1)]

\item  Continuous modeling of the seasonality using the proposed real-time concept and seasonal-trend model also allows for relating remote sensing to a continual process of land surface phenology change instead of a single date extraction \citep{Verbesselt:2010wo,White2006}. Further testing, however, is required to assess the impact of phenological shifts (e.g. changes in the start/end-of-season) on the method. Here, we demonstrated the method for forested areas where seasonal NDVI amplitude and the impact of phenological shifts are relatively small when compared to grassland areas, for example. Further testing is required for areas with a dominant herbaceous vegetation layer (e.g. grasslands, agriculture, and savannah areas). The proposed concept however has the capacity to detect phenological shifts in near-real time based on similar simulation experiment as proposed by \citet{Verbesselt:2010wo}.
...
\item the length of the stable history period could be used the restrict analysis to specific regions.
