...
The use of the real-time monitoring approach is demonstrated using the 16-day MODIS NDVI composites with a 5.6~km spatial resolution (MOD13QC1 collection~5). This product provides  NDVI corrected for the effect of atmospheric gases, thin cirrus clouds and aerosols as a proxy for global scale vegetation dynamics and disturbances \citep{Huete2002}.
The MOD13C1 global satellite images were acquired from February 2000 to July 2011 and were analysed for a study area covering Eastern Africa, Somalia. We used the MODIS quality assurance (QA) flags to select only cloud-free data of optimal quality. A NDVI time series was only analysed when either (1) it lacked less than 15\% of observation masked out by the QA flags and (2) it was covering vegetated land, specifically, having a median of NDVI time series larger than 0.2 NDVI \citep{Beurs2009}. For comparison with the simulation experiment, the noise level ($\sigma$) remaining after the season-trend model fit was estimated by deriving the standard deviation of the residuals from the model (Eq. \ref{eq:lmod}) of the stable history period.

%Also, the analysis was further restricted to vegetated zones by limiting the analysis to pixels where the median of NDVI time series was larger than 0.2 NDVI .
%The noise level remaining after the season-trend model fit was estimated to compare the performance of the method with the obtained results from the simulation experiment. The noise level is estimated by deriving the standard deviation of the residuals from the fitted seasonal-trend model (Eq. \ref{eq:lmod}) of the stable history period.

\section{Results and discussion}

\subsection{Simulation experiment} \label{sec:DiscSim}

Fig.~\ref{fig:SimNr} illustrates the probability for detecting a break within the monitoring period of a time series while varying the noise level ($\sigma$), magnitude of simulated disturbance ($m$) and the amount of data available in the monitoring period ($d$). The seasonal amplitude ($a$) did not influence the probability for break detection and results shown in Fig.~\ref{fig:SimNr}  are valid for the different simulated $a$'s (Table~\ref{table:simpar}). The length of the identified stable history period also did not influence the probability for break detection (results not shown). Furthermore, Fig.~\ref{fig:SimNr} shows that a break with $m = -0.4$ can be detected when $\sigma$ is smaller than 0.1 and $d = 4$ (i.e., 4 images available).  When the magnitude of the break is larger (e.g., $m = -0.6$), less data points ($d<4$) are required in the monitoring period for a similar amount of noise (e.g., $\sigma < 0.1$) in the history period. Three items can already be discussed; (1) importance of simulating data, (2) the purpose of this method, and (3) usefulness of the stable history period.
...
The main purpose of applying the real-time monitoring approach to MODIS 16-day NDVI time series is to illustrate a proof-of-concept of real-time disturbance detection. First, we limited break detection to land cover types with a mimimum amount of vegetation by limiting the analysis to regions with a median NDVI time series > 0.2. This also corresponds to major agricultural areas in Somalia (reference FOA land cover map of MODIS land cover map) 

% first explain about median NDVI threshold, then about the stable history, then look at the detected changes within those regions and also the noise level for further understanding in combination with the simulation results.

% ik weet nog niet of ik die length of stable history erbij zal zwieren - we kunnen eventueel hier een verhaal aan vast hangen van recent drought effects in non-irrigated zones in Somalia (does this correspond to the river delta in Somalia or not?).

We recommend limiting the disturbance detection to time series which have stable history length longer than 2 years (i.e. 46 16-day observations in this study) to enable OLS estimation of the 8 seasonal-trend model parameters (Eq. \ref{eq:lmod}).


% look at MODIS based land cover map -  but the FAO is better since these are shape files that are more easily plotted on top of the raster images.

Fig.~\ref{fig:SpatNoise} shows the spatial variation of the noise level ($\sigma$) in the study area. The lower noise levels correspond to the low vegetated areas or forested areas whereas the higher noise levels correspond to the semi-arid ecosystems (e.g. woodland and savannah). Higher noise levels the semi-arid ecosystems are caused by a large seasonal NDVI amplitude and inter-annual phenological shifts (e.g., shifts in the start and end of the growing season) due rainfall and temperature dynamics.


While the data and methods used here are appropriate for proof-of-concept development \citep{White2006}, other applications (e.g., deforestation monitoring in tropical forests or oil spill detection) will mandate further testing.
...
Fig.~\ref{fig:SpatTimeofChange} illustrates the time of the detected changes in 2010-2011. 




%Results shown in Fig.~\ref{fig:SpatTimeofChange} and~\ref{fig:SpatLStableHist} are illustrated for three locations within the study area in Fig.~\ref{fig:realmonitoring} and~\ref{fig:shorthistory}.
...
%Continuous modelling of the seasonality using the proposed real-time concept and seasonal-trend model allows for relating remote sensing to a continual process of land surface phenology change instead of a single date extraction \citep{Verbesselt:2010wo,White2006}.  

(1) The real-time monitoring could be further improved by; (a)~including other explanatory variables in the monitoring model (e.g., temperature, rainfall)
...
(2) To be included in the above paragraph: Climate tipping points etc. might be an nice reference \citep{Lenton:2011ke} and illustration of current ideas and potential use of 


% discuss the effect of the temporal resolution of the data
 
% compare our method with the disturbance detection algorithm of Potter2003!!! Discuss. Have a look at Potters results. - we already have done this in the introduction!
