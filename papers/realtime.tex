...
independent reference sources for a broad range of potential changes must be available during the change interval \citep{Kennedy2007}.
...
The objective of the time series simulation was to validate 
(1) how much new data is required to detect a disturbance,
(2) the accuracy of time of change estimation,  of the real-time change detection approach while varying the amount of noise, seasonal amplitude, and the magnitude of the disturbance in the simulated time series. NDVI time series were simulated using a similar approach as proposed by  \citet{Verbesselt:2010wo}. Simulated NDVI time series are generated by summing individually simulated seasonal, abrupt, and noise components (Fig.~\ref{fig:SimSetup}). First, the seasonal component is created using an asymmetric Gaussian function with a amplitude ($a$) for each season. Second, an abrupt change was added to the trend component to simulate an abrupt disturbance (e.g., fire, flood or harvest operation). This was simulated by combining a step function with a magnitude ($m$) and fixed gradient recovery phase. Third, the noise component was generated using a random number generator that follows a normal distribution N($\mu = 0$, $\sigma =x$). Vegetation index specific noise was generated by randomly replacing the white noise by noise with a value of $-0.1$, representing cloud contamination that often remains after atmospheric correction and cloud masking procedures. 

 \begin{figure}[htp]
\centering
    \includegraphics[height=0.6\textwidth]{figs/Sim_Monitoringsetup.eps}
  \caption{Simulated 16-day MODIS NDVI time series with a seasonal amplitude = 0.2, $\sigma$ = 0.05, containing one simulated abrupt change with $m$ = $-$0.3) (Table~\ref{table:simpar}). The simulated data series is the sum of the simulated seasonal, abrupt and noise series (black), and is used as an input in real-time monitoring method.  During the simulation experiment the period from 2000 until end 2005 is considered the history period and the year 2006 is the monitoring period that contains a simulated abrupt change.}
  \label{fig:SimSetup}
\end{figure}


The real-time change detection approach requires (a) a \emph{history period} i.e. data that already has been acquired and which will be analysed for stability in order to model normal vegetation dynamics, (b) a \emph{monitoring period} i.e. the period representing new data that recently has been captured which needs to be analyse whether something a change (i.e. disturbance) is occurring. Here, NDVI time series were simulated with an arbitrary length of 7 years  (i.e. 2000--2006) containing a simulated abrupt change in 2006, where the period 2000-2005 is the history period and the year 2006 the monitoring period (Fig.~\ref{fig:SimSetup}).
The objective is to test and see how much data is required before the abrupt change in the monitoring period can be detected while varying the noise, seasonal amplitude and the magnitude of the abrupt change.


We simulated 16-day NDVI time series with one abrupt disturbance in the monitoring period (Fig.~\ref{fig:SimSetup}). The 16-day NDVI time series are simulated by extracting key characteristics from MODIS 16-day NDVI time series within the study area \citep{Verbesselt2009a}. We selected a range of amplitude, noise, and $m$ values for the simulation study to represent a large range of land cover types of different data quality  while varying the amount of data available in the monitoring period (Table~\ref{table:simpar}). 1000 iterations of all the combinations of amplitude, noise, $m$, and amount of data in the monitoring period were performed to quantify; (1) the probability of detecting a disturbance in the monitoring period and (2) accuracy of estimating the timing of the detected disturbance change within a time series expressed as Root Mean Square Error (RMSE). Fig.~\ref{fig:SimMonitor} illustrates the simulation experiment.

 \begin{figure}[htp]
\centering
    \includegraphics[height=0.6\textwidth]{figs/Sim_Monitoring.eps}
  \caption{Simulated 16-day MODIS NDVI time series with a seasonal amplitude = 0.2, $\sigma$ = 0.05, containing one simulated abrupt change with $m$ = $-$0.3) (Table~\ref{table:simpar}). The simulated data series is the sum of the simulated seasonal, abrupt and noise series (black) (Fig.~\ref{fig:SimSetup}), and is used as an input in real-time monitoring method.  During the simulation experiment the period from 2000 until end 2005 is considered the history period and the year 2006 is the monitoring period that contains a simulated abrupt change.}
  \label{fig:SimSetup}
\end{figure}
