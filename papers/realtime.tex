...
possible when new data is captured. Validation and accuracy assessment of the real-time change detection method is done by 
...
the method to real 16-day MODIS satellite NDVI time series. Validation of multi-temporal change-detection methods is often not straightforward, since
independent reference sources for a broad range of potential changes must be available during the change interval \citep{Kennedy2007, Verbesselt2009a}.

We simulated 16-day NDVI time series with different noise levels, seasonality, and disturbances in order to robustly test the real-time change detection capacity in a controlled environment. However, it is challenging to create simulated time series that approximate remotely sensed time series, because these contain combined information on vegetation phenology, inter-annual climate variability, disturbance events, sensor conditions (e.g., viewing angle), and signal contamination (e.g., clouds) \citep{Zhang2009}. Therefore, applying the method to remotely sensed data remains necessary. 
...
The objective of the time series simulation was to validate (1) the accuracy of the real-time change detection approach and (2) how much new data is required before a disturbance can be detected while varying the amount of noise, seasonal amplitude, and the magnitude of the disturbance in the simulated time series. The real-time change detection approach requires (a) a \emph{history period} i.e. data that already has been acquired and which will be analysed for stability in order to model normal vegetation dynamics, (b) a \emph{monitoring period} i.e. the period representing new data that recently has been captured which needs to be analyse whether something normal or abnormal is happening. 

Here, NDVI time series were simulated with an arbitrary length of 7 years  (i.e. 2000--2006) containing a simulated abrupt change in 2006, where the period 2000-2005 is the history period and the year 2006 the monitoring period. %(see Fig. which illustrates the set-up). 
The objective is to test and see how much data is required in 2006 before the abrupt change in the monitoring period (2006) can be detected while varying the noise, seasonal amplitude and the magnitude of the abrupt change.

NDVI time series were simulated using a similar approach as proposed by  \citet{Verbesselt2009a, Verbesselt:2010wo}. Simulated NDVI time series are generated by summing individually simulated seasonal, noise, and trend components. First, the seasonal component is created using an asymmetric Gaussian function for each season \citep{Verbesselt:2010wo}. Second, the noise component was generated using a random number generator that follows a normal distribution N($\mu = 0$, $\sigma =x$). Vegetation index specific noise was generated by randomly replacing the white noise by noise with a value of $-0.1$, representing cloud contamination that often remains after atmospheric correction and cloud masking procedures. Third, an abrupt change was added to the trend component to simulate an abrupt disturbance (e.g., fire, flood or harvest operation). This was simulated by combining a step function with a specific magnitude (i.e. $m$) and fixed gradient recovery phase (See Fig. XX).

We simulated 16-day NDVI time series with one abrupt disturbance in the monitoring period (Figue~\ref{fig:Simults}). The 16-day NDVI time series are simulated by extracting key characteristics from MODIS 16-day NDVI time series within the study area \citep{Verbesselt2009a}. We selected a range of amplitude, noise, and $m$ values for the simulation study to represent a large range of land cover types of different data quality  while varying the amount of data available in the monitoring period(Table~\ref{table:simpar}). 1000 iterations of all the combinations of amplitude, noise, $m$, and amount of data in the monitoring period were performed to quantify; (1) the probability of detecting a disturbance in the monitoring period and (2) accuracy of estimating the timing of the detected disturbance change within a time series expressed as Root Mean Square Error (RMSE). 

% add figure to explain simulation strategy

\begin{table}[t!]
\caption{Parameter values ($a$, $\sigma$ noise and $m$) for simulation of 16-day NDVI time series while varying the amount of data available in the monitoring period ($n$).}
\centering
\begin{tabular}{ll}
  \hline
  ...
  \hline
  $a$         & $0.1, 0.3, 0.5$ \\
  $\sigma$ noise    & $0.01,0.02,\dots,0.07$ \\
...
We selected the 16-day MODIS NDVI composites with a 250m spatial resolution (MOD13Q1 collection~5), since this product provides frequent information at the
spatial scale at which the majority of human-driven land cover changes occur \citep{Townshend1988}. The MOD13Q1 16-day composites were generated using a
constrained view angle maximum NDVI value compositing technique \citep{Huete2002}. The MOD13Q1 images were acquired from 24 February 2000 to 14 September
2009 for a multi-purpose forested study area (\emph{Pinus radiata} plantation) in South Eastern Australia (Lat.~35.5$^\circ$~S, Lon.~148.0$^\circ$~E). The
abrupt and gradual changes occurring within the trend component of the MODIS satellite image time series for this study area are described by
\citet{Verbesselt2009a}. Furthermore, the study area consists out of two very different land cover types, i.e., grasslands and plantations, providing time
...
We used the binary MODIS Quality Assurance flags to select only cloud-free data of optimal quality. Moreover, a pixel time series was only selected for analysis when it lacked less than 10\% of the data \citep{Beurs2009}. The signal-to-noise ratio of the 16-day MODIS NDVI image series was derived by using the STL method 

% add info the STL decomposition to derive the signal to noise ratio
% we should redo the analyse without linear interpolation in order to verify whether it still works!!! - This is the aim of applying it on real data i.e. DEMONSTRATION!!!


The timing of the detected abrupt changes in the monitoring period (i.e. 2006) revealed were discussed and compared with the signal-to-noise ratio.
...
\begin{figure}[htp]
\centering
    \includegraphics[height=0.8\textwidth]{figs/realmonitoring.eps}
  \caption{ plot nr 4 and 35}
