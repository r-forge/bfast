...
Whereas the potential of the method has been illustrated for drought detection in Eastern Africa, it was developed as a generic multi-scale multi-purpose disturbance detection approach. Drought related disturbances were detected by restricting the analysis to areas with a minimum amount of vegetation cover and by requiring a minimum length of 2 years of the automatically detected stable history period. The method is able to identify drought effects with well in advance and can be used as an extra monitoring tool on top of other existing early warning tools. Further work is needed for operational implementation of the method to enable a rapid response by applying it to time series data with a high temporal resolution (e.g., hourly or daily data).
...
The MOD13C1 global satellite images were acquired from February 2000 to July 2011 and were analysed for abnormal changes, i.e. disturbances, \emph{from 2010 onwards} covering Eastern Africa, Somalia. We used the MODIS quality assurance (QA) flags to select only cloud-free data of optimal quality. A NDVI time series was only analysed when either (1) it lacked less than 15\% of observation masked out by the QA flags and (2) it was covering vegetated land, specifically, having a median of NDVI time series larger than 0.2 NDVI \citep{Beurs2009}. The noise level ($\sigma$) of NDVI time series was estimated by deriving the standard deviation of the residuals from the fitted seasonal-trend model (Eq. \ref{eq:lmod}) on the stable history period.  
...
While the data and methods used here are appropriate for proof-of-concept development for global scale disturbance monitoring, specific applications (e.g., drought or deforestation monitoring) will mandate further testing. Specialised climate- and satellite data based indices have been developed for drought monitoring \citep[e.g.][]{Funk:2009vf}. Here, however, we demonstrate how the method can be used as a generic and global early warning system by analysing MODIS 16-day NDVI time series.

%Fig. x a
First, Fig. \ref{fig:MedianNDVI} illustrates the spatial variation of the median of a NDVI time series (2000-2011) as a proxy for vegetation cover during the period of analysis. The disturbance detection was limited to areas with a minimum amount of vegetation cover by restricting the analysis to time series with a median NDVI $>0.2$ \citep{Beurs2009} which corresponds to major agricultural areas in Somalia (reference FOA Africover map). NDVI time series with no or very low NDVI values do not show any seasonality related to normal vegetation dynamics and are mainly influenced by variations in soil properties (e.g. moisture content). Other more specific vegetation indices are available for desert like conditions \citep[e.g. SAVI][]{Huete2002}, however, here we analysed NDVI time series as a proxy for global dynamics of photosynthetic capacity \citep{Myneni1995}. 

%Fig. x b
Second, Fig. \ref{fig:SpatLStableHist} illustrates the spatial variation of the length of the automatically identified stable history period (i.e. going backward in time from 2010) and is used to restrict the analysis to regions with a stable history longer than 2 years. The length of the stable history is identified for modelling normal, stable data variation within the history period to enable differentiation from abnormal changes in the monitoring period (i.e. 2010 onwards). The method for identifying the stable history is based on the structural change detection methods \citep{Bai2003,Zeileis:2010tt} and is implemented to assess stability of the history period. It is does not accurately determine  the time of change but verifies whether or not a structural change is occurring while stepwise going back in history \citep{Zeileis2002}. On the other hand, the BFAST approach is developed to accurately estimate time, magnitude, and direction of change within historical time series but it is not developed to detect disturbances in new observations (i.e. near real-time monitoring) \citep{Verbesselt2009a}. The length of the stable history as shown in Fig. \ref{fig:SpatLStableHist} could be used to have a rough indication of where in history the last major disturbance approximately occurred. More research, however, is required to validate and optimise the capacity of stable history identification for this purpose.

%Fig. x c
Third, Fig.~\ref{fig:SpatNoise} shows the spatial variation of the noise level ($\sigma$) in the study area. The lower noise levels correspond to the low vegetated areas or forested areas whereas the higher noise levels correspond to the semi-arid ecosystems (e.g. woodland and savannah). Higher noise levels in the semi-arid ecosystems can be explained by larger inter-annual phenological shifts when compared to areas with low vegetation cover or forests (e.g., shifts in the start and end of the growing season driven by rainfall and temperature variations)  \citep{Verbesselt:2010wo}. The noise range (i.e. 0--0.15) in Fig.~\ref{fig:SpatNoise} corresponds to the noise range of the simulation experiment (Fig.~\ref{fig:SimNr}) and illustrates that a wide range of terrestrial ecosystems were simulated for testing the proposed method in a controlled environment.

The magnitude of change is estimated by deriving the difference between the median of fitted seasonal-trend model and new observations during the monitoring period.

%Fig. xd magnitude of change ---
Finally, Fig.~\ref{fig:SpatTimeofChange} illustrates the magnitude of the detected changes in the monitoring period for areas where the stable history period is longer than 2 years and where the median of NDVI time series is larger than 0.2 NDVI. Major drought related anomalies are detected which correspond with agricultural areas of Eastern Somalia.  % but also other areas with a positive change magnitude are detected which only illustrate potentially not all regions are under drought stress in Eastern Africa.

Detected changes occur in areas with higher noise level (i.e. higher seasonality) which means that by looking at the results for the simulation experiment (Fig. X) that major disturbances are being detected (i.e. $m> -0.2$ NDVI at $\sigma>0.1$ and $d>4$). The magnitude levels of the disturbances detected in Fig. x d are lower due to a difference in the method to determine the magnitude of change in the simulation experiment (magnitude of the introduced step-change) and the real data (See section X).  


Fig. c and d illustrate together noise levels and areas of detected changes

Results shown in Fig.~\ref{fig:SpatLStableHist},~\ref{fig:SpatNoise}, and~\ref{fig:SpatTimeofChange} are illustrated for three locations within the study area in Fig.~\ref{fig:realmon}. Fig.~\ref{fig:realmon} are examples for specific locations in the study area illustrating three cases; (1) a detected disturbance with the full history period, (2) a case were no disturbance is detected, (3) a detected disturbance for a time series with a shorter stable history period.

Drought related disturbances in photosynthetic capacity of vegetation shown in Fig. X are only illustrating one potential stress factor contributing to the current political, structural, and social situation explaining the major famine occurring in Somalia at the moment (i.e. 2011) \citep{Funk:2011fg}. 
...
Currently, real-time satellite images and their derivatives are routinely produced and published on the World wide Web (e.g., \url{http://rapidfire.sci.gsfc.nasa.gov/} or \url{http://www.dmcii.com/index.html}) but methods for real-time monitoring and change detection are lacking. Here, we propose a time series based real-time monitoring approach that enables the detection of disturbances within a wide range of terrestrial ecosystem using satellite image time series. The change detection approach is a generic, data driven approach based on statistical principles applicable to different types of time series (e.g., in-situ or satellite sensors). The method can be applied to time series with a higher temporal resolution (e.g., hourly, daily or 8-day time series) to enable rapid response to detected disturbances. 

The method has been demonstrated in a controlled environment by simulating time series containing disturbances and by applying it to MODIS NDVI image time series of covering Eastern Africa, Somalia. 

Results from the simulation experiment demonstrate that the noise level within the time series together with the amount of data available in the monitoring period are the main drivers of the capacity to detect disturbances within newly acquired images. 
...
Overall, the results illustrate that major disturbances can be detected in highly variable ecosystems (e.g. savannah or woodlands) whereas the disturbance detection is more accurate in more stable ecosystems (e.g. deforestation in tropical forests) 
...
\begin{figure}
\centering
 \subfloat[][] {\includegraphics[height=0.5\textwidth]{figs/MedianNDVI_Somalia.png} } %
 \subfloat[][] {\includegraphics[height=0.5\textwidth]{figs/LengthHistoryPeriod_EA.png} } \\
 \subfloat[][] {\includegraphics[height=0.5\textwidth]{figs/Noise_EA.png} } %
 \subfloat[][] {\includegraphics[height=0.5\textwidth]{figs/MagnAbnormalBreak_201012_EA.png} }
 \caption{test}
 \label{fig:spatial}
\end{figure}
...
%\begin{figure}[htp]
%\centering
%    \includegraphics[height=0.7\textwidth]{figs/TimeofAbnormalBreak_2010_EA}
...
\begin{figure}[htp]
\centering
    \includegraphics[height=0.8\textwidth]{figs/tsexampleSOM.eps}
  \caption{
 Results of real-time monitoring approach applied to 16-day MODIS NDVI time series for three locations (a),(b), and (c). 
