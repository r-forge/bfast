...
The MOD13C1 global satellite images were acquired from February 2000 to July 2011 and were analyzed for disturbances, \emph{from mid 2010 onwards} covering Eastern Africa, Somalia. 

\readme{illustrate main land cover types using the MODIS IGBP land cover classification maps and illustrate climate data trends to confirm drought effect in Eastern Africa}
...
While the data and methods used here are appropriate for proof-of-concept development for global scale disturbance monitoring, specific applications (e.g., drought or deforestation monitoring) will mandate further testing. Specialized climate- and satellite data based indices have been developed for drought monitoring \citep[e.g.][]{Funk:2009vf}. Here, however, we demonstrate how the method can be used as a near real-time disturbance detection method by analyzing MODIS 16-day NDVI time series covering Eastern Africa.

First, Fig. \ref{fig:spatiala} illustrates the median of a NDVI image time series (2000-2011) as a proxy for vegetation cover during the period of analysis. The disturbance detection was limited to areas with a minimum amount of vegetation cover by restricting the analysis to time series with a median NDVI $>0.2$ \citep{Beurs2009}. NDVI time series with no or very low NDVI values do not show any seasonality related to normal vegetation dynamics and are mainly influenced by variations in soil properties (e.g. moisture content). Other vegetation indices (e.g. SAVI) exist which reduce influence from soil back ground \citep{Huete2002}, however, here we analyzed NDVI time series as a proxy for global dynamics of photosynthetic capacity \citep{Myneni1995}. 

Second, Fig. \ref{fig:spatialb} illustrates the spatial variation of the length of the identified stable history period and is used to restrict the analysis to regions with a stable history longer than 2 years. The length of the stable history is identified for modeling normal, stable data variation within the history period to enable differentiation from abnormal changes in the monitoring period (i.e. mid 2010 onwards). The method for identifying the stable history is based on the structural change detection methods \citep{Bai2003,Zeileis:2010tt} and is implemented to assess stability of the history period. It is does not accurately determine  the time of change but verifies whether or not a structural change is occurring while stepwise going back in history \citep{Zeileis2002}. The BFAST approach is developed to accurately estimate time, magnitude, and direction of change within historical time series but it is not developed to detect disturbances in new observations (i.e. near real-time monitoring) \citep{Verbesselt2009a}. The length of the stable history as shown in Fig. \ref{fig:spatialb} could be used to have a rough indication of where in history the last major disturbance approximately occurred. More research, however, is required to validate and optimize the capacity of stable history identification for this purpose.

Third, Fig.~\ref{fig:spatialc} shows the spatial variation of the noise level ($\sigma$) in the study area. The lower noise levels correspond to the low vegetated areas or forested areas whereas the higher noise levels correspond to the semi-arid ecosystems (e.g. woodland and savannah). Higher noise levels in the semi-arid ecosystems can be explained by larger inter-annual phenological shifts when compared to areas with low vegetation cover or forests (e.g., shifts in the start and end of the growing season driven by rainfall and temperature variations)  \citep{Verbesselt:2010wo}. The noise range (i.e. 0--0.15) in Fig.~\ref{fig:spatialc} corresponds to the noise range of the simulation experiment (Fig.~\ref{fig:SimNr}) and illustrates that the range of terrestrial ecosystems NDVI dynamics in the study area were simulated for testing the proposed method in a controlled environment.

Finally, Fig.~\ref{fig:spatiald} illustrates the magnitude of the detected disturbances in the monitoring period for areas where the stable history period is longer than 2 years and where the median of NDVI time series is larger than 0.2 NDVI. The magnitude shown in Fig. is an indication of the magnitude of the disturbance and other approaches should be used for a more accurate change magnitude estimation \citep[e.g. BFAST][]{Verbesselt2009a} once more data is available.

Negative NDVI disturbances are detected related to the current ongoing drought in Eastern Africa \citep{Funk:2011fg}. Detected changes occur in areas with higher noise level (i.e. higher seasonality) and as indicated by the simulation experiment (Fig.~\ref{fig:SimNr} that disturbances with a large magnitude are occurring (i.e. $m> -0.2$ NDVI at $\sigma>0.1$ and $d>4$).

Drought related disturbances in photosynthetic capacity of vegetation shown in Fig.~\ref{fig:spatiald} and~\ref{fig:realmon} are illustrating one of stress factors besides political instability and lack of infrastructure contributing the major famine currently occurring in Somalia \citep{Funk:2011fg}. 
...
\centering
  ...
  \caption{Simulated 16-day MODIS NDVI time series with $a$ = 0.4, $\sigma$ = 0.05, containing one simulated abrupt change with $m$ = $-$0.3). The period from 2004 until mid 2010 (i.e., the time step just before the simulated break), is considered the \emph{history period} and the period after the simulated break is the \emph{monitoring period} (grey background). The monitoring period contains 6 observations  ($d$ = 6, \textcolor{red} {red} line). The result of the monitoring approach are shown. A stable history period is identified within the history period (i.e. 2007 until mid 2010) and used to model and predict the normal data variation (\textcolor{blue} {blue} dashed line) to enable disturbance detection (Section~\ref{sec:MonStrucChange}).  Here, a disturbance is detected (\textcolor{OliveGreen} {green} line).}
...
\begin{figure}
\centering
 \subfloat[][] {\label{fig:spatiala} \includegraphics[height=0.5\textwidth]{figs/MedianNDVI_Somalia.eps} } %
 \subfloat[][] {\label{fig:spatialb} \includegraphics[width=0.5\textwidth]{figs/LengthHistoryPeriod_EA.eps} }  \\
 \subfloat[][] {\label{fig:spatialc} \includegraphics[width=0.5\textwidth]{figs/Noise_EA.eps} } %
\subfloat[][] {\label{fig:spatiald} \includegraphics[width=0.5\textwidth]{figs/MagnAbnormalBreak_201012_EA.eps} }
...
\centering
  ...
  \caption{
 Results of real-time monitoring approach applied to 16-day MODIS NDVI time series for three locations a, b, and c, which correspond to symbols $+,~\triangle$~and~$\diamondsuit$ shown in Fig.~\ref{fig:spatial}.}
