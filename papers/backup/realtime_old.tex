...
The objective of the time series simulation experiment was to validate 
...
Fig.~\ref{fig:SimNr} illustrates the probability for detecting a break within the monitoring period of a time series (i.e. the newly acquired data) while varying the noise level, $m$ (i.e. magnitude of simulated disturbance in the monitoring period), and $n$ the amount of data available in the monitoring period. The seasonal amplitude ($a$) did not influence the probability for break detection and results shown in Fig.~\ref{fig:SimNr}  are valid for the different simulated $a$'s (Table~\ref{table:simpar}). Fig.~\ref{fig:SimNr} furthermore shows that a break with $m = -0.2$ can be detected when noise level is smaller than 0.25 and $n = 3$ (i.e. 3 images available).  


% the time estimation of the method is not tested here as the method is not specifically develloped for accurrate data of the time of change but for detecting something abnormal in the montoring period e.g. newly acquired data.
...
The main purpose of applying the method onto real data is to illustrate how the method can be used. Further testing is required to operationally implement the method for detection of changes in specific land cover types (e.g. grasslands).

illustrates results detected changes in 2006, the noise levels and also the length of the stable history period.
Noise levels was the main driver of change detection accuracy in the monitoring period and therefore was illustrated here. Fig. X illustrates the length of the stable history period this indicates when the last abnormal change occurred in the history period. stable history period have a minimum length of 1, which inidicates that a change recently occurred in the history period or that the time series is unstable or not reliable.

to do check a time series where the length in smaller than one...
