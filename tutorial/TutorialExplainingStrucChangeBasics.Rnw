...
\title{Tutorial: Detecting a breakpoint a time series}
\author{Jan Verbesselt, Achim Zeileis}

\maketitle

\begin{abstract}
This tutorial explain the basics of structural change test within time series data.
Basic principles are illustrated using the \emph{Nile} data set included in the strucChange package.
\end{abstract}
