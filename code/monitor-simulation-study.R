...
nrobs <- 159
sdnoise <- 0.01
dip <- -0.2; # vary size of dip between 0 and 0.15 in steps of 0.01
noisef <- 3 # vary noise factor by 1 - 3 by steps of 0.05
a <- 0.2
dfend <- 1  # vary between 0 and 22 data points from the end
...
#           #savepng("figs/Sim_Monitoringsetup2")
...
#           #dev.off()
          ...
          abline(v=time(ftsNDVI)[nrobs-dfend],col='blue',lty=2,lwd=2)
          lines(tshistory,col='green',lwd=2)
          ...
          #savepng("figs/Sim_Monitoring",pointsize=9)
            ...
            legend("bottomleft",
            c("History","Monitoring","Stable History","fit based on stable history",
            "Time of Simulated Break", "Time of Detected Break")
              ,lty=c(1,2,NA,1,2,2),col=c(1,1,'blue','blue','blue','red'),pch=c(NA,NA,19,NA,NA,NA))
          #dev.off()   
          ## output for accuracy assessment
          ## simulation setting that are important to be saved to an output file
      out <- data.frame(dip,noisef,n.range,a, dfend, Tsim = time(ftsNDVI)[nrobs-dfend], Tmon = tbp)
      write.table(out,"outputsim.csv", append=TRUE, sep=",", col.names= FALSE)
      }
    }
  }
}
